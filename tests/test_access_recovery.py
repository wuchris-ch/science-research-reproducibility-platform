...
def test_team_scope_including_artifacts_events_and_cache(service):
    p = locked(service)
    r = service.submit("alice", RunCreate(plan_id=p["id"]), "one", "image")
    ...
    app = create_app(service.settings, Sandbox())
    identity = {"sub": "outsider"}
    app.dependency_overrides[app.state.auth.actor] = lambda: identity["sub"]
    with TestClient(app) as c:
        for path in [
            f"/api/runs/{r['id']}",
            f"/api/plans/{p['id']}",
            f"/api/runs/{r['id']}/export",
            f"/api/runs/{r['id']}/artifacts/metrics.json",
            f"/api/workspaces/{p['workspace_id']}/events",
        ]:
        ...
        assert (
            c.post("/api/plans", json={"workspace_id": p["workspace_id"], "title": "Denied"}).status_code
            == 403
        )
...
def test_oidc_signature_issuer_audience_and_expiry(service, monkeypatch):
    ...
    monkeypatch.setattr(
        app.state.auth.jwks, "get_signing_key_from_jwt", lambda token: SimpleNamespace(key=key.public_key())
    )
    base = {
        "sub": "alice",
...
def test_receipt_survives_crash_after_termination(service):
    r = queued(service)
    sandbox = Sandbox()
    worker = Worker(service, sandbox, "first")
    run = worker.claim()
    attempt = worker.attempt(run)
    assert worker.save_attempt(
        run,
        {
            **attempt,
            "phase": "sealed",
            "outcome": "succeeded",
            "receipt": {"artifacts": {}, "diagnostic": "sealed"},
        },
    )
    with service.db.transaction() as c:
    ...
    with service.db.transaction() as c:
        assert service.get_run(c, "alice", r["id"])["state"] == "succeeded"
...
def test_other_docker_host_cannot_recover_ephemeral_sandbox(service):
    ...
    worker.claim()
    with service.db.transaction() as c:
        c.execute(update(runs).values(lease_until=0))
    other = Worker(service, Sandbox(), "two")
    other.engine_id = "different-runtime"
...
def test_backup_restore_validates_blobs_and_separates_active_jobs(service, tmp_path):
    ...
    with service.db.transaction() as c:
        row = service.get_run(c, "alice", r["id"])
        body = {**row["body"], "artifacts": {"output": {"sha256": blob, "bytes": 14}}}
        c.execute(update(runs).where(runs.c.id == r["id"]).values(body=body))
    snapshot = tmp_path / "backup"
    backup(service.settings, snapshot)
    ...
    with copy.db.transaction() as c:
        assert copy.get_run(c, "alice", r["id"])["state"] == "failed"
    assert copy.store.read(blob) == b"durable result"
    (snapshot / "blobs" / blob).write_bytes(b"changed")
    with pytest.raises(ValueError):
        restore(service.settings, snapshot, tmp_path / "corrupt-restore")
