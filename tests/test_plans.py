...
from workbench.schemas import PlanCreate, Correction, Parameters, LockRequest, RunCreate
from workbench.service import Problem, REQUIRED_REVIEW
...
def test_revisions_and_lock(service):
    ...
    with pytest.raises(Problem):service.correct('alice',p['id'],patch.model_copy(update={'expected_revision':3}))
...
def test_concurrent_idempotency_and_scope(service):
    ...
    assert e.value.status==409
    with service.db.transaction() as c:
        with pytest.raises(Problem):service.get_run(c,'bob',result[0]['id'])
...
def test_injected_evidence_cannot_correct_plan(service):
    ...
    with pytest.raises(Problem):service.correct('alice',draft['id'],Correction(expected_revision=1,parameters=Parameters(),reason='New claim',evidence_ids=['invented']))
    with pytest.raises(Problem):service.new_plan('alice',PlanCreate(workspace_id=w,title='Variation',parent_id=p['id'],reason='test'))
