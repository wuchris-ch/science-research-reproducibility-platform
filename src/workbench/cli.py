...
def prepare_annotation(settings):
    ...
    if not ledger.exists():
        raise RuntimeError("Historical annotation manifest is not available")
    entry = json.loads(ledger.read_text())
    archive = settings.data_dir / "sources/chen-annotation.tar.gz"
    ...
    with tarfile.open(archive) as tf:
        ...
        if not member.isfile() or member.size > 300_000_000:
            raise ValueError("Invalid annotation database")
    ...
    with sqlite3.connect("file:" + str(dbpath) + "?mode=ro", uri=True) as c:
        rows = c.execute(
            "SELECT genes.gene_id, gene_info.symbol FROM genes JOIN gene_info USING (_id) ORDER BY genes.gene_id"
        ).fetchall()
    import csv
...
def build(settings):
    ...
    for src, name in [
        ...
        ("fixtures/law-samples.csv", "law-samples.csv"),
    ]:
...
def backup(settings, target: Path):
    if not settings.database_url.startswith("sqlite:///"):
        raise RuntimeError("Use pg_dump plus an artifact snapshot for PostgreSQL; see operations guide")
    if target.exists():
        raise ValueError("Backup destination already exists")
    target.mkdir(parents=True, mode=0o700)
    original = settings.database_url.removeprefix("sqlite:///")
    with sqlite3.connect(original) as source, sqlite3.connect(target / "workbench.db") as dest:
        source.backup(dest)
    ...
    (target / "README.txt").write_text(
        "Consistent metadata snapshot plus immutable artifacts. Sessions are intentionally excluded.\nRestore into a new directory using workbench restore. In-flight jobs must be reconciled.\n"
    )
    print(target)
...
def restore(settings, source: Path, target: Path):
    ...
    with service.db.transaction() as c:
        for r in c.execute(select(runs)).mappings():
            for a in r["body"].get("artifacts", {}).values():
                service.store.read(a["sha256"])
        # A restored install must not assume ownership of the source installation's active containers.
        for r in c.execute(
            select(runs).where(runs.c.state.in_(["running", "queued", "cancel_requested"]))
        ).mappings():
            c.execute(
                update(runs)
                ...
                .values(
                    ...
                    body={
                        **r["body"],
                        "diagnostic": "Restored from backup; source runtime ownership was not transferred",
                    },
                )
            )
            service.db.emit(
                c,
                r["workspace_id"],
                "run.restore_interrupted",
                "restore",
                {"reason": "Submit a fresh run after review"},
                r["id"],
            )
    ...
    if args.command == "setup":
        ...
        while True:
            try:
                ...
                logging.exception("Worker operation failed; durable state retained for reconciliation")
            if args.once:
                return
            time.sleep(1)
    elif args.command == "backup":
        backup(settings, args.destination.resolve())
    elif args.command == "restore":
        restore(settings, args.source.resolve(), args.destination.resolve())
    elif args.command == "export":
        from .exports import bundle, verify_bundle

        service = make_service(settings)
        with service.db.transaction() as c:
            run = service.get_run(c, "local", args.run_id)
