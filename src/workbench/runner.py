...
class Docker:
    ...
    def launch(self,name,run):
        ...
        args=['create','--name',name,'--platform','linux/amd64','--network','none','--read-only',
              '--user','1000:1000','--cap-drop','ALL','--security-opt','no-new-privileges',
              '--pids-limit','96','--memory',f"{limits['memory_mib']}m",'--memory-swap',f"{limits['memory_mib']}m",
              '--cpus',str(limits['cpu']),'--ulimit','nofile=256:256','--ulimit','fsize=67108864:67108864',
              '--tmpfs','/work:rw,nosuid,nodev,noexec,size=128m,uid=1000,gid=1000,mode=0700',
              ...
              '--label','research.workbench=true','--label',f"research.run={run['id']}",
              '--label',f"research.epoch={run['epoch']}",
              '-e','PLAN_JSON='+canonical(run['body']['plan']).decode(),
              '-e',f"WALL_SECONDS={limits['wall_seconds']}",run['body']['image_id']]
...
class Worker:
    ...
    def claim(self):
        ...
        with self.db.transaction() as c:
            ...
            run.update(values)
            if queued:
                c.execute(insert(attempts).values(run_id=run['id'],epoch=epoch,body={'started':now,'container':self.name(run),'phase':'intent'}))
                self.db.emit(c,run['workspace_id'],'run.started',self.identity,{'epoch':epoch},run['id'])
            return run
    def owned(self,c,run):
        current=self.db.row(c,runs,run['id'])
        return current and current['owner']==self.identity and current['epoch']==run['epoch'] and current['state'] in ('running','cancel_requested')
    def finish(self,run,state,extra):
        with self.db.transaction() as c:
            if not self.owned(c,run):return False
            current=dict(self.db.row(c,runs,run['id']))
            if current['cancel_requested']:state='cancelled'
            c.execute(update(runs).where(runs.c.id==run['id']).values(state=state,body={**current['body'],**extra},
                      owner=None,lease_until=0,updated=time.time()))
            self.db.emit(c,run['workspace_id'],'run.'+state,self.identity,{'epoch':run['epoch'],'diagnostic':extra.get('diagnostic')},run['id'])
        return True
    def attempt(self,run):
        with self.db.transaction() as c:
            return c.execute(select(attempts.c.body).where(attempts.c.run_id==run['id'],attempts.c.epoch==run['epoch'])).scalar_one()
    def save_attempt(self,run,body):
        with self.db.transaction() as c:
            if not self.owned(c,run):return False
            c.execute(update(attempts).where(attempts.c.run_id==run['id'],attempts.c.epoch==run['epoch']).values(body=body))
        return True
    def retry_or_fail(self,run,diagnostic):
        ...
        with self.db.transaction() as c:
            if not self.owned(c,run):return False
            c.execute(update(runs).where(runs.c.id==run['id']).values(state='queued',owner=None,lease_until=0,updated=time.time()))
            self.db.emit(c,run['workspace_id'],'run.retry_queued',self.identity,{'reason':diagnostic,'epoch':run['epoch']},run['id'])
    def tick(self):
        run=self.claim()
        ...
        if attempt.get('phase')=='sealed':
            ...
            if self.finish(run,attempt['outcome'],attempt['receipt']):self.docker.remove(name)
            return True
        info=self.docker.inspect(name)
        if run['cancel_requested']:
            self.docker.stop(name)
            if self.finish(run,'cancelled',{'diagnostic':'Sandbox termination confirmed'}):self.docker.remove(name)
            return True
        if info is None:
            ...
            return True
        if info['Config']['Labels'].get('research.run') != run['id'] or info['Image'] != run['body']['image_id']:
        ...
        if info['State']['Running'] and self.docker.done(name):
            ...
            receipt={'artifacts':artifacts,'exit_code':code,'duration_seconds':time.time()-attempt['started'],
            ...
            if code==0:
                try:
                    ...
                    outcome='failed';receipt['diagnostic']='Output validation failed: '+str(e)
            else:receipt['diagnostic']='Time limit exceeded' if code==124 else f'Scientific process exited with status {code}'
            if self.save_attempt(run,{**attempt,'phase':'sealed','receipt':receipt,'outcome':outcome}):
                self.docker.stop(name)
                if self.finish(run,outcome,receipt):self.docker.remove(name)
            return True
        if not info['State']['Running']:
            diagnostic='Memory limit exceeded' if info['State'].get('OOMKilled') else 'Sandbox exited before completion signal'
            # OOM is a deterministic budget failure, not an automatic spend/retry trigger.
            if self.finish(run,'failed',{'diagnostic':diagnostic,'container_state':info['State']}):self.docker.remove(name)
            return True
        if time.time()-attempt['started'] > run['body']['limits']['wall_seconds']+60:
            self.docker.stop(name)
            if self.finish(run,'failed',{'diagnostic':'Supervisor wall-clock limit exceeded'}):self.docker.remove(name)
        return True
