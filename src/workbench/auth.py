...
class Auth:
    def __init__(self,settings):
        ...
        if any((settings.oidc_issuer,settings.oidc_audience,settings.oidc_jwks_url)) and not all((settings.oidc_issuer,settings.oidc_audience,settings.oidc_jwks_url)):
    ...
    def actor(self,request: Request):
        ...
        if self.jwks:
            if not bearer.startswith('Bearer '):raise Problem(401,'OIDC access token required')
            token=bearer[7:]
            try:
                key=self.jwks.get_signing_key_from_jwt(token)
                claims=jwt.decode(token,key.key,algorithms=['RS256'],audience=self.settings.oidc_audience,
                                  issuer=self.settings.oidc_issuer,options={'require':['exp','iat','sub']})
            except Exception:raise Problem(401,'Invalid identity token') from None
        else:
            try:
                claims=jwt.decode(request.cookies.get('research_session',''),self.key,algorithms=['HS256'],
                                  audience='research-local',options={'require':['exp','sub','csrf']})
            except jwt.PyJWTError:raise Problem(401,'Start a local session') from None
            if request.method not in ('GET','HEAD','OPTIONS') and not secrets.compare_digest(request.headers.get('x-csrf-token',''),claims['csrf']):
                raise Problem(403,'CSRF token required')
        subject=claims.get('sub')
        if not isinstance(subject,str) or not 1<=len(subject)<=200:raise Problem(401,'Invalid subject')
        return subject
