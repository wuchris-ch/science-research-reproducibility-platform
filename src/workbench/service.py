...
from .database import Database, workspaces, members, plans, revisions, runs, events, reviews, uid
from .fixtures import SOURCES, EVIDENCE
from .schemas import PlanCreate, Correction, LockRequest, RunCreate
...
class Service:
    ...
    def authorize(self, c, workspace_id, actor, minimum='viewer'):
        role = c.execute(select(members.c.role).where(members.c.workspace_id == workspace_id,
                                                     members.c.subject == actor)).scalar()
    ...
    def new_plan(self, actor, request: PlanCreate):
        ...
        body['adaptations'] = ['Linux instead of the original operating system; rendering fonts may differ',
            'Entrez IDs retained; Law display-only symbol annotation omitted'] if request.dataset_id == 'law2018' else [
            'R and edgeR differ from the 2016 paper; original annotation and filter are preserved']
        body['reviewed_fields'] = []
        with self.db.transaction() as c:
            self.authorize(c,request.workspace_id,actor,'editor')
            if request.parent_id:
                parent = self.get_plan(c,actor,request.parent_id)
                if parent['workspace_id'] != request.workspace_id or parent['body']['dataset_id'] != request.dataset_id:
            ...
            identity = uid()
            c.execute(insert(plans).values(id=identity,workspace_id=request.workspace_id,revision=1,
                       state='draft',body=body,created=time.time()))
    ...
    def record_revision(self,c,identity,revision,body,actor):
        c.execute(insert(revisions).values(plan_id=identity,revision=revision,body=body,actor=actor,created=time.time()))
    ...
    def correct(self,actor,identity,request: Correction):
        with self.db.transaction() as c:
            ...
            body = {**plan['body'],'parameters':request.parameters.model_dump(),'reason':request.reason,
                    'evidence_ids':request.evidence_ids,'reviewed_fields':[]}
            revision = plan['revision']+1
            changed = c.execute(update(plans).where(plans.c.id==identity,plans.c.revision==request.expected_revision,
                             plans.c.state=='draft').values(revision=revision,body=body)).rowcount
            if changed != 1:
                raise Problem(409,'Concurrent plan update')
            self.record_revision(c,identity,revision,body,actor)
            self.db.emit(c,plan['workspace_id'],'plan.corrected',actor,{'plan_id':identity,'revision':revision,'reason':request.reason})
            return self.get_plan(c,actor,identity)
    ...
    def lock(self,actor,identity,request: LockRequest):
        with self.db.transaction() as c:
            ...
            if set(request.reviewed_fields) != set(REQUIRED_REVIEW):
                raise Problem(422,'Review dataset, samples, filter, environment and comparison before locking')
            body = {**plan['body'],'reviewed_fields':REQUIRED_REVIEW,'locked_by':actor,'locked_at':time.time()}
            body['plan_hash'] = digest(body)
            changed = c.execute(update(plans).where(plans.c.id==identity,plans.c.revision==request.expected_revision,
                    plans.c.state=='draft').values(state='locked',body=body,revision=plan['revision']+1)).rowcount
            if changed != 1:
                raise Problem(409,'Concurrent plan update')
            self.record_revision(c,identity,plan['revision']+1,body,actor)
            self.db.emit(c,plan['workspace_id'],'plan.locked',actor,{'plan_id':identity,'hash':body['plan_hash']})
            return self.get_plan(c,actor,identity)
    ...
    def submit(self,actor,request: RunCreate,key: str,image_id: str):
        with self.db.transaction() as c:
            ...
            c.execute(select(workspaces.c.id).where(workspaces.c.id==plan['workspace_id']).with_for_update())
            existing = c.execute(select(runs).where(runs.c.workspace_id==plan['workspace_id'],runs.c.key==key)).mappings().first()
            if existing:
                if existing['request_hash'] != req_hash:
                    raise Problem(409,'Idempotency key already used for a different request')
                return dict(existing)
            pending = c.execute(select(func.count()).select_from(runs).where(runs.c.workspace_id==plan['workspace_id'],runs.c.state.in_(['queued','running','cancel_requested']))).scalar()
            if pending >= 8:
                raise Problem(429,'Workspace queue limit reached')
            identity, now = uid(), time.time()
            cache_key = digest({'plan':plan['body'],'image':image_id,'limits':request.limits.model_dump()})
            body = {'plan':plan['body'],'limits':request.limits.model_dump(),'image_id':image_id,
                    'cache_key':cache_key,'artifacts':{},'comparison':None,'submitted_by':actor}
            state = 'queued'
            if request.use_cache:
                for previous in c.execute(select(runs).where(runs.c.workspace_id==plan['workspace_id'],runs.c.state=='succeeded').order_by(runs.c.created.desc())).mappings():
                    if previous['body']['cache_key']==cache_key:
                        ...
                        state='succeeded'
                        break
            c.execute(insert(runs).values(id=identity,workspace_id=plan['workspace_id'],plan_id=plan['id'],key=key,
                       request_hash=req_hash,state=state,epoch=0,lease_until=0,cancel_requested=0,body=body,created=now,updated=now))
            self.db.emit(c,plan['workspace_id'],'run.cached' if state=='succeeded' else 'run.queued',actor,{'run_id':identity},identity)
    ...
    def cancel(self,actor,identity):
        with self.db.transaction() as c:
            ...
            c.execute(update(runs).where(runs.c.id==identity).values(cancel_requested=1,state=state,updated=time.time()))
