...
from workbench.database import plans, workspaces
from workbench.runner import Docker, Worker
from workbench.schemas import LockRequest, Parameters, PlanCreate, RunCreate
from workbench.service import REQUIRED_REVIEW

s = make_service(Settings())
docker = Docker(s.settings)
worker = Worker(s, docker)
...
for recipe, dataset, title, parameters in [
    ...
    with s.db.transaction() as c:
        p = c.execute(select(plans).where(plans.c.workspace_id == w)).mappings().all()
    ...
    if p is None:
        p = s.new_plan(
            ...
            PlanCreate(
                ...
                parent_id=baseline if parameters.filter_policy == "cpm1" else None,
                reason="Test sensitivity to a stricter expression threshold"
                if parameters.filter_policy == "cpm1"
                else "",
            ),
        )
        p = s.lock(
            "local", p["id"], LockRequest(expected_revision=p["revision"], reviewed_fields=REQUIRED_REVIEW)
        )
...
while time.time() < deadline:
    worker.tick()
    with s.db.transaction() as c:
        current = [s.get_run(c, "local", i) for i in submitted]
    if all(r["state"] not in ("queued", "running", "cancel_requested") for r in current):
        break
    time.sleep(0.5)
for r in current:
    ...
    if r["state"] != "succeeded":
        for n in ["stderr.log", "stdout.log"]:
            if n in r["body"]["artifacts"]:
                print(s.store.read(r["body"]["artifacts"][n]["sha256"]).decode(), flush=True)
    ...
    "runs": [
        {
            "id": r["id"],
            "plan_id": r["plan_id"],
            "recipe": r["body"]["plan"]["recipe"],
            "dataset": r["body"]["plan"]["dataset_id"],
            "filter": r["body"]["plan"]["parameters"]["filter_policy"],
            "state": r["state"],
            "comparison": r["body"]["comparison"],
